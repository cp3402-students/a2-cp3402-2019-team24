<?php
/**
 * Coffeetime functions and definitions
 *
...
if ( ! function_exists( 'coffee_setup' ) ) :
	...
	function coffee_setup() {
		 ...
		 * If you're building a theme based on Coffeetime, use a find and replace
		...
		add_theme_support( 'custom-background', apply_filters( 'coffee_custom_background_args', array(
...
function coffee_scripts() {
	wp_enqueue_style( 'coffee-style', get_stylesheet_uri() );

	wp_enqueue_script( 'coffee-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'coffee-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
